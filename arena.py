...
class Arena:
    ...
    def __init__(self, problem_templates: ProblemTemplates, llms):
        ...
        self.competion_scores = CompetitionScores(len(llms))

    def run(self, n_rounds=None, n_problems=1):
        ...
        self.logger.info(f'Number of problems: {n_problems}')
        self.logger.info(f'Models under evaluation: {self.model_names}')
        self.logger.info('')

        start_time = now()

        problems = self.__build_problems(n_problems)

        self.logger.info('Starting competition rounds')
        for n in range(n_rounds):
            master, student = random.choice(pairs)
            master_index = self.model_name_to_index[get_model_name(master)]
            student_index = self.model_name_to_index[get_model_name(student)]
            problem_index = random.randint(0, n_problems - 1)
            problem = problems[problem_index]
            self.logger.info('-------------------------------------------------------------------------------')
            self.logger.info(f'Round: {n + 1}, master: {get_model_name(master)}, student: {get_model_name(student)}')

            self.logger.info(f'Problem to solve: #{problem_index + 1}')
            ...
            self.competion_scores.update(master_index, student_index, scores)
    ...
    def show_results(self):
        ...
        student_avg_scores = self.competion_scores.get_student_avg_score()
        ...
        master_avg_scores = self.competion_scores.get_master_avg_score()
        for n in range(n_llms):
            llm = self.llms[n]
            model_name = get_model_name(llm)
